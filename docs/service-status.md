...
Prepare auth first. For the status portion of this release, deploy the compatible website UI before the API/monitor begins emitting the eighth component: the new UI accepts both seven- and eight-component feeds, while the old UI rejects eight. Then deploy the website API and restart any independent monitor worker on the matching code. This ordering follows the status response dependency; database preparation still comes first.
